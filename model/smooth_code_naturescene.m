function smooth_code_naturescene(varargin)
% slightly revised main_SWR_reference_no_cnw.m
% for Project 2 attention, add sensory input as external spikes rather than
% external current

dt = 0.1;
sec = round(10^3/dt); % 1*(10^3/dt) = 1 sec

step_tot = 400*sec;%7.1*sec; % use 10 second!
...
dg_K = 0.003;
for g_balance = 0.98;
    for ii = 1:length(d_Image)
        qi_contrast = 0.8;
        qi_c_gradient = 0;%linspace(-1,1,11)
                                                        ...
                                                        for rate_ext_E = [0.85 ]; %
                                                            for  tau_c_EE = [8]
                                                                ...
                                                                for tau_c_I = [20]
                                                                    ...
                                                                    %[FID] = new_GCin_files_and_randseedHDF5(loop_num,repeats);
                                                                    ...
                                                                    %rate_ext_on_base1 = zeros(1,step_tot);
                                                                    %rate_ext_on_base1(1:4e4-1) = 1; % duration of stimulus
                                                                    ...
                                                                    % current stimulus
                                                                    ImageName = d_Image(ii).name;
                                                                    writeExtCurrentPopHDF5(FID,ImageName,1);
                                                                    ...
                                                                    % outset initial membrane potential
                                                                    %     file_name = 2;
                                                                    ...
                                                                    %%%%%%%%% add stimuli as external current
                                                                    ...
                                                                    % writeNeuronSamplingHDF5(FID, 1, [0,1,1,1,0,0,0,1], sample_neuron, sample_steps )
                                                                    % writeNeuronSamplingHDF5(FID, 2, [0,1,1,1,0,0,0,1], 1:N(2), sample_steps )
                                                                    % Yifan's
                                                                    ...
                                                                    % real-time COM recording
                                                                    sample_steps = zeros(1,step_tot);
                                                                    sample_steps(1:end) = 1;
                                                                    writeRealTimeCOMHDF5(FID, 1, [1 1], sample_steps)
                                                                    ...
                                                                    writeExplVarHDF5(FID, 'discard_transient', discard_transient, ...
                                                                        ...
                                                                        'sti_start_step', h5read(d_Image(ii).name,'/start_step'),...
                                                                        'sti_end_step', h5read(d_Image(ii).name,'/end_step'),...
                                                                        'g_balance',g_balance);
